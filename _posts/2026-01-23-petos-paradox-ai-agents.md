...
So, if a model has a 90% success at a task lasting 1hr, and a 50% chance at a task lasting 3hrs, the logistic fit (from these two points alone!) would force a very specific, optimistic shape onto the tail, potentially predicting that failure is effectively impossible at 10 minutes or that performance collapses completely at 4 hours, regardless of the underlying mechanics of failure.
...
Secondly, if the reported estimates I present are real, the real path to agentic success will not necessarily be in reducing the constant hazard (that seems easy enough, bigger models!), but in training models to reduce their hazard over time, like humans. The human $k$ is still quite substantially smaller than the models tested by METR at that time, and there was little apparent association between model size and $k$.<span class="sidenote">We should be cautious here. Limited data on smaller models that failed a lot means model fits are a bit dodgy. I don’t think we can overinterpret this.</span>

Finally, and this is a point well made by others, but highlighted by this data: we should be sceptical of any model that makes the assumption that the median is like the tails. This is not true in many fields and is actually a very strong assumption. The Weibull fits I present show vastly different estimates for tasks that extremely long/difficult. I am not suggesting these are right, but I am pretty confident in this case that the tails are likely to be quite different, and that modelling this differently will give different quite different answers.
